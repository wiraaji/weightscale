...
void printWeight() {
  ...
  PRINTER.print(prevWeightKg, 3);  // Print the current weight
  PRINTER.print(" Kg^FS");
  ...
  // currentBgColor = TFT_GREEN;
  // Clear the "Printing" and "Done" messages
  tft.fillRect(140, 170, 180, 40, TFT_GREEN);  // Adjust the coordinates and size of the rectangle to clear the text area
  delay(500);
}
    ...
    if (buttonPressed) {
      ...
      if (pressDuration >= 8000) {
        ...
        if (weightKg >= BATAS_BAWAH && weightKg <= BATAS_ATAS) {
          ...
          delay(1000);
          // Clear the message area while maintaining the current background color
...
void updateLCD(float weightGrams) {
  ...
  // Perbarui tampilan LCD hanya jika perubahan berat lebih besar dari atau sama dengan tolerance
  if (abs(weightKg - prevWeightKg) >= tolerance) {
    // Bulatkan berat menjadi 4 digit dibelakang koma
    weightKg = round(weightKg * 10000) / 10000.0;
    ...
    } else if (weightKg >= 0.199 && weightKg <= 0.201) {
      weightKg = 0.200;
      digitalWrite(BUZZER, MATI);
